...
[R][R]や[R Commander（以降、Rcmdr）][Rcmdr]に限ったことではありませんが、一般的に分析過程の記録というのは残しにくいものです。[データ分析勉強会][1]で主に用いた[Rcmdr][Rcmdr]やExcelのようなGUI操作のものは、操作自体は簡単ですが、それらを使った分析過程を記録するには大変な手間がかかります。例えば[Rcmdr][Rcmdr]を例に挙げると  
　  
...
　  
つまり、分析結果を報告書にまとめるためには分析と並行して常に手順や結果を記録しておく必要があります。このような作業は間違いを誘発することが容易に想像できます。これを防止して間違いのない分析・報告書作成を行う確実な方法は、分析と報告書作成が同時にできる[R Markdown][RMD]を利用することです。  
　  

# R Markdownとは
[R Markdown][RMD]は文字通り（軽量）マークアップ言語である
[Markdown][MD]による記述を用いた記録方法で、[R][R]におけるドキュメント作成フレームワークです。[R Markdown][RMD]を用いるとドキュメントの中にデータやコード、分析結果を埋め込むことが可能になります。  
　  
...
　  
[R Makrdown][RMD]を利用するとドキュメントを生成するたびにデータが読み込まれ、[R][R]のコードが実行され、グラフや計算結果、表などが出力され、自動的にドキュメントに埋め込まれます。これによりグラフや計算結果、表などをドキュメント修正のたびに切り貼りし直す必要もなく[Markdown][MD]による章構成も可能で、常に指定したデータから同じ手順で同じ結果を得ることが可能になります。つまり、[R Markdown][RMD]を用いることで

* ドキュメント生産性の向上（コピペ汚染からの解放、簡単作成）
* 分析過程の可読性確保
* 分析再現性の確保（[Reproducible Research][CRAN_TV_RR]）

が実現できるようになります。更に[R Markdown][RMD]では、同一ソースから複数のフォーマットのドキュメントを作成することも可能ですので、提出先に応じた形式のドキュメント作成も簡単です。  
　  
...
# R Markdownの基本
[R Markdown][RMD]は[Pandoc](https://pandoc.org/)の[Markdown][MD]をベースとした記述を用いますので、ファイル形式はシンプルなテキスト形式になります。なお、日本語を利用する場合には言語コードとしてBOMなしの**UTF-8**を利用することを推奨します。  
...
## Markdown {#Markdown}
[R Markdown][RMD]で使える[Markdown][MD]は前述のように[Pandoc](https://pandoc.org/)の[Markdown][MD]です。[R Markdown][RMD]の記述ルールは多岐に渡りますので、代表的なルールを以下に紹介します。  
