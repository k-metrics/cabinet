...
[R <i class="fa fa-external-link"></i>][R]や[R Commander（以降、Rcmdr） <i class="fa fa-external-link"></i>][Rcmdr]に限ったことではありませんが、一般的に分析過程の記録というのは残しにくいものです。[データ分析勉強会 <i class="fa fa-external-link"></i>][1]で主に用いた[Rcmdr <i class="fa fa-external-link"></i>][Rcmdr]やExcelのようなGUI操作のものは、操作自体は簡単ですが、それらを使った分析過程を記録するには大変な手間がかかります。例えば[Rcmdr <i class="fa fa-external-link"></i>][Rcmdr]を例に挙げると  
　  

* 操作が自体が記録に残らない（実行された結果は残せる）
...
　  
つまり、分析結果を報告書にまとめるためには分析と並行して常に手順や結果を記録しておく必要があります。このような作業は間違いを誘発することが容易に想像できます。これを防止して間違いのない分析・報告書作成を行う確実な方法は、分析と報告書作成が同時にできる[R Markdown <i class="fa fa-external-link"></i>][RMD]を利用することです。  
　  

# R Markdownとは
[R Markdown <i class="fa fa-external-link"></i>][RMD]は文字通り（軽量）マークアップ言語である
[Markdown <i class="fa fa-external-link"></i>][MD]による記述を用いた記録方法で、[R <i class="fa fa-external-link"></i>][R]におけるドキュメント作成フレームワークです。[R Markdown <i class="fa fa-external-link"></i>][RMD]を用いるとドキュメントの中にデータやコード、分析結果を埋め込むことが可能になります。  
　  
...
## 使うためのには
[R Markdown <i class="fa fa-external-link"></i>][RMD]を使うには必要なソフトウェアが同梱されている[RStudio <i class="fa fa-external-link"></i>][RS]環境で使うのが現状ではベストです。[RStudio <i class="fa fa-external-link"></i>][RS]のインストールは[こちら](../program/env_index.html)を参照してください。[RStudio <i class="fa fa-external-link"></i>][RS]  
[**`rmarkdown`**パッケージ <i class="fa fa-external-link"></i>](https://cran.r-project.org/web/packages/rmarkdown/index.html){target="_blank" title="Dynamic Documents for R"}が必須ですので、以下のコマンドまたは[RStudio <i class="fa fa-external-link"></i>][RS]のGUIなどからインストールしてください。  
...
# R Markdownの基本
[R Markdown <i class="fa fa-external-link"></i>][RMD]は[Pandoc <i class="fa fa-external-link"></i>](https://pandoc.org/){target="_blank" title="a universal document converter"}の[Markdown <i class="fa fa-external-link"></i>][MD]をベースとした記述を用いますので、ファイル形式はシンプルなテキスト形式になります。なお、日本語を利用する場合には言語コードとしてBOMなしの**UTF-8**を利用することを推奨します。  
...
このテンプレートからドキュメントを生成するとこのページを作成してい環境ではこのようなドキュメントになります。  
![R Markdown Documents <i class="fa fa-external-link"></i>](./fig/rmd_default_template.png)
　  
このように[R Markdown <i class="fa fa-external-link"></i>][RMD]の記述には見慣れない記号などがありますが、記述としては大きく以下の３つパートから構成されますので、この構成さえ覚えれば[R Markdown <i class="fa fa-external-link"></i>][RMD]を記述するのは簡単です。

* 出力フォーマットや体裁を記述する[YAML](#YAML)部
* [R][R]のコードを記述する[(Code) Chunk](#Chunk)部
* 文書を記述する[Markdown <i class="fa fa-external-link"></i>](#Markdown)部
...
^2)^ **`collapsed`**, **`smooth_scroll`**を指定する場合は何も指定しない  
...
### インライン・チャンク
ドキュメント中で[R <i class="fa fa-external-link"></i>][R]のコード実行したい場合にはインライン・チャンクを用います。インライン・チャンクはバッククォートで囲んだ（$`r `$）中にコードを記述します。例えば、チャンクの中で円の面積を求めた以下のようなコードがある場合  
　  
