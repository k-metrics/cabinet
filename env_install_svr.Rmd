...
Rの統合開発環境（Integrated Development Environment, IDE）のデファクトスタンダードともいえるRStudioにはサーバー版の [RStudio Server <i class="fa fa-external-link"></i>](https://www.rstudio.com/products/rstudio/#Server){target="_blank" title="RStudio"} が用意されています。RStudio ServerはWebサーバーベースなのでOS環境（プラットフォーム）への依存性が最小限です。コンテナ技術と併用することでプラットフォームをクリーンに保ち、環境の共有が楽になるというメリットがあります。ここでは最小限の知識と手間でRStudio Serverの環境を構築する方法を紹介します。
...
## "rocker", the Docker Container
[rocker <i class="fa fa-external-link"></i>](https://hub.docker.com/u/rocker/){target="_blank" title="Docker Hub"} というDockerコンテナが公開されています。rockerはベーシックなR単体のコンテナをベースに様々なバリエーションが作られています。代表的なコンテナは下表の通りです。  
...
この他に機械学習用のコンテナも用意されています。ただし、全てのコンテナには日本語ロケールや日本語フォントが含まれていませんので、日本語が必要な場合には追加が必要です。
...
## Dockerのインストール
Dockerのインストールに関しては公式ドキュメント [ <i class="fa fa-external-link"></i>](https://docs.docker.com/install/overview/){target="_blank" title="docker docs"} を参考にしています。
...
ダウンロードとパッケージの解凍が完了したら以下のコマンドでコンテナイメージが作成されていることを確認します。
```{bash, eval=FALSE}
sudo docker images
```

コンテナイメージが確認できたらコンテナを以下のコマンドを使って起動します。
```{bash, eval=FALSE}
...
`-d`   | コンテナをバックグラウンドで起動します（デタッチドモード）が最初は使わない方が動きが分かりやすいと思います
`-p`   | 公開するポート番号を「ホスト側:コンテナ側」の形式で指定します
`--rm` | コンテナ終了時に自動的にコンテナと関連するファイルシステムなどを削除します（デタッチドモードでは指定できません）
`-v`   | ホスト側ディレクトリ（絶対パス）をコンテン側ディレクトリ（絶対パス）にマウントします

`[host-dir]` - コンテナ環境にマウントしたいホスト側のディレクトリ（絶対パス）
`[mp]` - コンテナ側のマウントポイント（絶対パス）
    `rocker/*`のコンテナイメージはデフォルトユーザが`rstudio`ですので`/home/rstudio`ディレクトリ配下にマウントポイントを作ることをおすゝめします。

`[password]` - RStudio Serverへのログインパスワード


コンテナが起動したらブラウザで`localhost:8787`にアクセスします。
...
コンテナで加えた変更（Rパッケージの追加、RStudioの設定変更）はコンテナを終了させると破棄されてしまいます。変更を保存するには`docker commit`コマンドを利用してコンテナイメージを保存してください。`ID`は`docker ps -a`コマンドで確認します。
```{bash, eval=FALSE}
...
# 日本語対応コンテナの作成
Tokyo.Rの有志によるrocker/tidyverseの日本語対応のコンテナが [tokyor/rstudio <i class="fa fa-external-link"></i>](https://hub.docker.com/r/tokyor/rstudio){target="_blank" title="Docker Hub"} で公開されていますが、残念ながらこの1年程更新がなく、Rのバージョンが3.4.3で止まっています。日本語対応のrockerが欲しい場合には公開されている [Dockerfile <i class="fa fa-external-link"></i>](https://hub.docker.com/r/tokyor/rstudio/dockerfile){target="_blank" title="tokyor/rstudio, Docker Hub"} を利用してローカルビルドすることにより日本語対応のrockerコンテナを作成してください。
...
`Dockerfike`の準備ができましたら以下のコマンドで、コンテナをビルドします。  
```{bash, eval=FALSE}
...
ビルドが完了しましたら、`sudo docker run`コマンドでコンテナを起動してみましょう。引数は前述の通りです。
