...
RStudio Desktopが各プラットフォームで抱えている固有の問題を解決することができます。このRStudio Serverを [Docker <i class="fa fa-external-link"></i>](https://www.docker.com/){target="_blank" title="Enterprise Container Platform for High Velocity Innovation"} のイメージ技術を用いることで、Linuxサーバや仮想環境を立ち上げることなくWindows（要Hyper-V）、macOS、Linuxの各プラットフォームのローカル環境上で単一環境のRStudioを実現することができます。
...
* [ローカルのRStudioの設定をDockerイメージ上のRStudio Serverに反映する <i class="fa fa-external-link"></i>](https://qiita.com/nozma/items/cae323708ff60cc2f67a){target="_blank" title="Qiita"} 
