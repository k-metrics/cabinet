...
`rvest`パッケージにはサイトのエンコードを推測する`rvest::guess_encoding`があります。例えば気象庁のページは`UTF-8`ですが、`rvest::guess_encoding`にスクレイピングしたいページの情報を丸ごと渡すと以下のような結果を返してくれます。  

```{r}
site_html %>% rvest::guess_encoding()
```

[シフトJISを使い続ける上場企業をまとめてみた <i class="fa fa-external-link"></i>](https://www.megamouth.info/entry/2017/10/20/015056){target="_blank" title="megamouthの葬列"} を参考に文字セットが`UTF-8`とは異なるサイトをチェックしてみると


#### 日清オイリオ（Shift_JIS）
![](./fig/nisshin_oilio.png)
```{r}
