...
スクレイピングは禁止
```{r, include=FALSE}
# site_url <- "https://coinmarketcap.com/ja/currencies/bitcoin/historical-data/"
# xpath_hs <- "//*[@id=\"historical-data\"]/div/div[2]/table"
```

```{r}
# site_url %>% 
#   xml2::read_html() %>% 
#   rvest::html_node(xpath = xpath_hs) %>% 
#   rvest::html_table()
```
...
makuuchi <- purrr::map2(site_url, c(1:15), ~ paste(.x, .y, sep = "")) %>% 
  purrr::map2(xpath_mu, 
              .f = function(.x, .y) {xml2::read_html(.x) %>% 
                                       rvest::xml_node(xpath = .y) %>% 
                                       rvest::html_table()})

makuuchi %>% purrr::map_df(.f = data.frame) %>% 
  tidyr::separate(`東.1`, into = c("e_name", "e_lose"), sep = "[勝]") %>% 
  dplyr::mutate(e_win = readr::parse_number(e_name)) %>% 
  tidyr::separate(e_name, c("e_name"), sep = "[[:digit:]]") %>% 
  dplyr::mutate(e_lose = readr::parse_number(e_lose)) %>% 
  dplyr::select(e_pos = `東`, e_name, e_win, e_lose)
```
