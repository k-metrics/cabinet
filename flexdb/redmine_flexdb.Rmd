...
(issues_2018 %>% 
  ...
  dplyr::mutate(category = as.character(category),
              category = dplyr::if_else(is.na(category), "Uncategorized",
                                        category),
              category = forcats::fct_relevel(category, cat_levels),
...
issues_2018 %>% 
  ...
  flexdashboard::valueBox(color = "danger")
```
...
(tmp_issues %>% 
  ...
  ggplot2::ggplot(ggplot2::aes(x = week)) + 
    ...
    # ggplot2::scale_fill_manual(values = c("#FB4870", "#FB4870", "#990066", "#000000", "#FF00FF"))
) %>% 
  plotly::ggplotly()
```
...
(issues_2018 %>% 
  ...
  ggplot2::ggplot(ggplot2::aes(x = close_h)) + 
    ggplot2::geom_bar(ggplot2::aes(fill = tracker)) + 
    ggplot2::labs(x = "Close time [H]", y = "Number of issues") +
    ggplot2::scale_fill_brewer(palette = "Accent")) %>% 
  plotly::ggplotly()
```
...
issues_2018 %>% 
  ...
  dplyr::count(author) %>% 
  dplyr::arrange(dplyr::desc(n)) %>% 
  dplyr::top_n(5) %>% knitr::kable(caption = "Open at extra hours, Worst 5")

issues_2018 %>% 
  dplyr::filter(close_h > close_at | close_h < open_at) %>% 
  dplyr::count(assigned_to) %>% 
  dplyr::arrange(dplyr::desc(n)) %>% 
  dplyr::top_n(5) %>% knitr::kable(caption = "Close at extra hours, Worst 5")
```
