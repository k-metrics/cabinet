...
## Coloumn R {data-width=900} {.tabset}
...
issues_2018 %>% 
  ...
  DT::datatable(options = list(pageLength = 15), caption = "Defect issues")
```
...
issues_2018 %>% 
  ...
  dplyr::count(category, status) %>% 
  tidyr::spread(status, n) %>% 
  DT::datatable(options = list(pageLength = 15), caption = "Patch issues")
```
...
issues_2018 %>% 
  ...
  dplyr::count(category, status) %>% 
  tidyr::spread(status, n) %>% 
  DT::datatable(options = list(pageLength = 15), caption = "Feature issues")
```
