...
　  
赤色の升（テキスト表示の場合は`0`の部分）が欠損値のある組み合わせになり`Ozone`変数単独の欠損値が最も多く、ついで`Solar.R`変数単独の欠損値が多いことが分かります。更に詳細な可視化が必要な場合は`VIM`パッケージを用います。  
　  
...
## 多重代入法
多重代入法は欠損値を疑似的に補完したデータセット（疑似完全データともいう）を複数作成し、統計分析を行う際に作成した全ての疑似完全データを用いて統計分析を行い、分析結果を統合して結論を出す方法です。一口に多重代入法といっても様々なアルゴリズムが提案されていますが、ここで扱う多重代入法はMICE（連鎖方程式による多変量補定）によるものです。  
...
#### 注意事項
多重代入法は乱数を用いますので、本ページでは`mice::mice`関数に`seed`オプションを指定して発生する乱数を固定することで再現性を確保しています。また、疑似完全データセットの数はデフォルトの`m = 5`を使用しています。疑似完全データセットの数は歴史的にはデフォルト値程度と言われてきたようですが、最近では`m = 20`くらいが目安と言われているようです。
参考までに`mice::mice`関数が計算した疑似完全データセットの密度分布（赤線）を下図にしめします。疑似完全データセット数が多い方が青線の元データ（欠損値を含むデータの密度分布）に近づいており大きく外れた値を計算していないように見えます。  
　  
...
<!-- ### データセット数 -->
<!-- `mice::mice`関数はデフォルトで`m = 5`セットのデータセットを作成します。 -->

<!-- https://www.slideshare.net/koichirogibo/ss-32418161 -->
