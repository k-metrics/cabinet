...
## 多重代入法
多重代入法は欠損値を疑似的に補完したデータセット（疑似完全データともいう）を複数作成し、全ての擬似完全データを分析し、それらの分析結果を統合して結論（推定値）を導き出す方法です。一口に多重代入法といっても様々なアルゴリズムが提案されていますが、ここで扱う多重代入法はMICE（連鎖方程式による多変量補定）によるものです。  
...
下図の平均値代入法（単一代入法）と比べて`airquality`データセットでは上図の多重代入法を用いた方がより適切な補完が行われていると言えます。  
　  
...
## 機械学習による補完
Rでは機械学習（ランダムフォレスト）による欠損値の補完を`imputeMissings`パッケージを用いて行うことができます。`imputeMissings::compute`関数を用いて学習データを作成し`imputeMissings::impute`関数で欠損値を求めますが、学習データの用意ができない場合でも対象データを元に演算を行ってくれます。  
　  
