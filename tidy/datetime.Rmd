...
lubridate | `r packageVersion('lubridate')` | Make Dealing with Dates a Little Easier
...
`lubridate::isoweek` | [ISO 8601 <i class="fa fa-external-link"></i>](https://ja.wikipedia.org/wiki/ISO_8601){target="_blank" title="Wikipedia"}の週番号（ISO weeks） | 月曜開始
...
　  
特にカレンダーをイメージした月内週数を得たい場合には`lubridate::isoweek`関数または`lubridate::epiweek`関数を使用する必要があります。  
　  
...
### 処理コード例
上記の処理コード例を以下に示しますが、引数の厳密なチェックを行っていませんので、もし、コードを利用する場合は引数に注意してください。  
　  
...
* [ISO 8601概説 <i class="fa fa-external-link"></i>](https://ja.wikipedia.org/wiki/ISO_8601){target="_blank" title="Wikipedia"}
* [What is an "EPI Week"? <i class="fa fa-external-link"></i>](https://www.cmmcp.org/season-summary-data/pages/what-epi-week){target="_blank" title="Central Mass. Mosquite Control Project"}
