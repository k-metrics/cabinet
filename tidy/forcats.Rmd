...
# forcatsパッケージ
前述のような場合に便利なのが`forcats`パッケージです。`forcats`パッケージには
...
## 水準の順番を変える
`forcats::gss_cat`データセットの`rincome`(reported income)は因子型変数ですが、その水準は以下のようになっています。  
　  

```{r}
forcats::gss_cat$rincome %>% 
  levels()
```

　  
これを逆にして収入の低い順から並べかえ、金額以外の回答を最後に並べるには`forcats::fct_rev`関数と`forcats::fct_relevel`関数を用います。
```{r}
x <- forcats::gss_cat

(x$rincome <- x$rincome %>% 
  forcats::fct_rev() %>%
  forcats::fct_relevel("Don't know", "Not applicable", "Refused", "No answer",
                       after = Inf)) %>% 
  levels()
```

　  
本当に水準（`levels`）が変更されているか層別グラフを描いて確認してみます。  
　  

```{r}
forcats::gss_cat %>% 
  ggplot2::ggplot(ggplot2::aes(x = rincome, y = tvhours)) +
...
　  
このように収入の低い順から高い順に並べ替えられているのが確認できます。  
　  

## 水準の値を変える
`forcats::gss_cat$rincome`の水準は\$10,000を境に区切り幅が異なっています。これを均等な区切り幅（\$5,000）に統一し、未回答などをまとめてみます。水準をまとめるには`forcats::fct_collapse`関数を用います。  
　  

```{r}
(x$rincome <- x$rincome %>% 
  forcats::fct_collapse(
    `to $4999` = c("Lt $1000", "$1000 to 2999", "$3000 to 3999","$4000 to 4999"),
    `$5000 to 9999` = c("$5000 to 5999", "$6000 to 6999", "$7000 to 7999",
                        "$8000 to 9999"),
    missing = c("Don't know", "Not applicable", "No answer")
  )) %>% levels()
...
x %>% 
  ...
  # ggplot2::ggplot(ggplot2::aes(x = rincome, y = tvhours)) +
    # ggplot2::geom_boxplot() +
    # ggplot2::coord_flip()
  ggplot2::ggplot(ggplot2::aes(x = tvhours, y = rincome, fill = rincome)) +
    # ggridges::geom_density_ridges_gradient(alpha = 0.65)
    # ggridges::geom_density_ridges(alpha = 0.65)
...
## 因子型変数の水準を確認する
因子型変数の水準を確認するには`levels`関数、`unique`関数または`forcats::fct_unique`関数を用います。`levels`関数と`forcats::fct_unique`関数の動作は等価で水準は水準順に表示されます。一方、`unique`関数は水準の出現順に水準を表示することに注意してください。  
　  
```{r}
