...
combo      | box, box_no_facet, dot, dot_no_facet, facethist, facetdensity, denstrip, blank | |  連続量（数値型）と離散量（因子型）の組合わせに対するグラフ形式
discrete   | facetbar, ratio, blank | barDiag, blankDiag | 離散量（因子型）に対するグラフ形式
...
dot          | 散布図（層別）                   |
dot_no_facet | 散布図                           |
facethist    | ヒストグラム（層別）             | `lower`のデフォルト値
facetdensity | 密度分布（層別）                 |
