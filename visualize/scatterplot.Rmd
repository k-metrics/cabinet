...
`car::scatterplot`関数ではデフォルトで回帰直線を描けますが明示的に指定する場合は`regLine`オプションに`"lm"`を指定してください（v2.1-6では`reg.line`オプションです）。なお、v3.0-0では散布図を層別描画するための`groups`オプションを指定し回帰直線を層別描画させないように`by.groups = FALSE`を指定すると散布図が正しく描けないようですので注意してください。  
　  
