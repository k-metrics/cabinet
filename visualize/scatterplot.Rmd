...
## 周辺箱ひげ図付き散布図
`car`パッケージの`car::scatterplot`関数は[R Commander <i class="fa fa-external-link"></i>][Rcmdr]で散布図を描く際に呼び出されている関数です。特徴は両軸に箱ひげ図（周辺箱ひげ図）を描がける点です。
...
car::scatterplot(Sepal.Width ~ Sepal.Length, data = iris, groups = iris$Species,
                 reg.line = FALSE, smooth = FALSE, spread = FALSE)
```
...
## 散布図と回帰線
散布図は前述のように相関関係や回帰分析の際に利用されますので回帰線を描ける必要があります。  
　  

```{r}
car::scatterplot(Sepal.Width ~ Sepal.Length, data = iris, by.groups = FALSE,
                 groups = iris$Species, smooth = FALSE, spread = FALSE)
```
...
```{r}
car::scatterplot(mpg ~ hp, data = mtcars, reg.line = lm, smooth = FALSE,
                 span = 0.5, spread = FALSE, boxplots = FALSE)
