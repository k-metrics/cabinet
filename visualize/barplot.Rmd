...
棒グラフは横軸に因子の水準を縦軸にその値や度数などを配置することで水準ごとの違いを可視化して比較することができます。横軸に時系列データを取ることで時系列の推移を可視化することも可能です。  
　  
...
## 統計量を比較する
`ggplot2::geom_bar`関数で描く棒グラフの縦軸はデフォルトで度数ですが、縦軸を統計量として描く場合は`stat`オプションを指定する必要があります。  
