...
# チケットの集計
データフレームに対する集計を行うには`dplyr::group_by`関数＋`dplyr::summarise`関数または`dplyr::count`関数を用いるのが便利です。`tidyr::spread`関数と組み合わせることでクロス集計表を作成しやすくします。  
　  
なお、以下の二つのコードは共に`data`に含まれる`key`の水準毎に個数を数える処理で結果は等価になります。  
　  
