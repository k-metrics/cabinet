...
# 基本的な使い方
本資料では`ggplot2`パアッケージのv2.2.1をベースに記述しています。`ggplot2`の文法は非常にシンプルです。まず、データと座標を用意し、その上に描きたいコンポーネントを重ねていくイメージです。GIS（地理情報システム）やPhotoshopにおけるレイヤー機能と同じような考え方です。先程の描画の例では
```{r}
iris %>% 
  ggplot2::ggplot(aes(x = Sepal.Width, y = Sepal.Length)) +
    ggplot2::geom_point()
...
`ggplot2`パッケージは関数を`+`記号を用いて連結していきます。レイヤーを`+`で重ね合わせるというイメージです。
```{r}
iris %>% 
  ggplot2::ggplot(aes(x = Sepal.Width, y = Sepal.Length)) + 
    ggplot2::geom_point()
...
iris %>% 
  ggplot2::ggplot(aes(x = Sepal.Length, fill = Species)) + 
    ggplot2::geom_histogram(bins = 20, position = "identity", alpha = 0.5)
```
...
## 箱ひげ図にデータを重ねる
箱ひげ図だけではデータの分布傾向が分かりませんのでデータを重ねて描画してみます。この場合は`ggplot2::geom_jitter`関数を用います。
```{r, eval=FALSE}
...
mtcars %>% 
  ggplot2::ggplot(aes(x = wt, y = mpg)) +
    ...
    ggplot2::geom_text(aes(label = row.names(.), colour = cyl), 
...
```{r}
mtcars %>% 
  ggplot2::ggplot(aes(x = wt, y = mpg, colour = cyl)) +
    ggplot2::geom_point() +
    ggplot2::geom_smooth(method = "lm", se = TRUE) +
