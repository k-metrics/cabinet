...
## ggplot2::aes関数のオプション
`ggplot2`を理解するコツの一つに`ggplot2::aes`関数でのオプション指定があります。

指定内容     | オプション | 指定例 | 備考
-------------|---------|----------------------------------------|----
座標軸の指定 | x, y, z | aes(x = Petal.Length, y = Petal.Width) | zは3次元の場合
色の指定     | colour  | aes(colour = Species)                  | 描画線のみ
塗りつぶ色の指定 | fill | aes(fill = Species)                   |
シンボルの指定   | shape | aes(shape = Species)                 | 
サイズの指定     | size  | aes(size = Species)                  | 
線種の指定       | linetype | aes(linetype = Species)           | 
透過度の指定     | alpha | 
