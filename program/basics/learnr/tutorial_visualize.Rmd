...
## Open-close chart
同様にパッチのオープンクローズチャートを描いてみましょう。
```{r open_close_patch, exercise=TRUE, exercise.lines=26}
bugs %>% 
  
```

```{r open_close_patch-hint}
# "Closed"チケット以外は全て"Open"チケット扱いとし作成日をオープン日(`Date`)とする
p_open <- bugs %>% 
  dplyr::select(ID = '#', Tracker = 'トラッカー', Status = 'ステータス',
                OpenDateTime = '作成日', CloseDateTime = '終了日') %>% 
  dplyr::mutate_at(vars(Status),
                   funs(replace(., (. != "Closed"), "Open"))) %>% 
  dplyr::filter(Tracker == "Patch" & Status == "Open") %>% 
  dplyr::mutate(Date = lubridate::as_date(OpenDateTime))
...
# `Date`を横軸の日付としてステータス毎に累積度数を計算し折れ線グラフで表示する
p_open %>% 
  dplyr::bind_rows(p_closed) %>% 
  dplyr::arrange(Date) %>% 
  dplyr::count(Date, Status) %>% 
  dplyr::group_by(Status) %>% 
  dplyr::mutate(Cumsum = cumsum(n)) %>% 
  ggplot2::ggplot(aes(x = Date)) +
    # ggplot2::geom_bar(aes(y = Cumsum, fill = Status), stat = "identity",
    #                       position = "dodge") +
    ggplot2::geom_line(aes(y = Cumsum, colour = Status))
```
...
```{r ggplot, exercise=TRUE, exercise.lines=12}
...
bugs %>% 
  ...
  ggplot2::ggplot(aes(x = Date, y = Count)) +
    ggplot2::geom_area(aes(fill = Status), stat = "identity", na.rm = TRUE)
