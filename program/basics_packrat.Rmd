...
<!-- shared Links -->
```{r collaboration, child="../shared/links.Rmd", include=FALSE}
```
...
# packratの使い方
**`packrat`**の使い方は[公式チュートリアル（英語） <i class="fa fa-external-link"></i>](http://rstudio.github.io/packrat/walkthrough.html){target="_blank" title="RStudio, Inc."}にありますが、公式チュートリアルのようにコマンドラインから使うよりもRStudioから利用するのが便利です。  
　  
...
　  
作成されたアーカイブファイルを共有先で展開すれば同じ環境が構築できます。  
　  
...
## プロジェクトの作成
プロジェクトをリポジトリを利用して共有している場合は［File］-［New Project...］-［Version Control］から利用しているリポジトリ（Gir or Subversion）を選択してリモートリポジトリをローカルのプロジェクトとしてクローンしてください。クローニングが完了すると自動的に**`packrat/init.R`**ファイルが実行されてpackrat環境の構築を始めます。利用しているパッケージ数やPC、通信環境にもよりますが十数分程度かかります。  
　  

## packrat情報の更新
プロジェクトのパッケージを更新したり追加、削除した場合は**`packrat::snapshot`**関数を用いてpackrat情報を更新します。更新された情報は**`packrat/packrat.lock`**ファイルに記録されますので、プロジェクトをリポジトリを利用して共有している場合は**`packrat/packrat.lock`**ファイルをリポジトリにpushしてください。  
ファイルを利用して共有している場合は前述の**`packrat::bundele`**関数を用いてアーカイブファイルを作成してください。  
　  

### packat情報の反映
プロジェクトをリポジトリを利用して共有している場合は更新された**`packrat/packrat.lock`**ファイルをpullした後に以下のコマンドを実行してパッケージライブラリを更新してください。  
　  
