...
1. Rtoolsをデフォルトオプション^1^でインストールします（Winodws Only）
...
![](./fig/packrat/packrat_option.png)
...
## packrat情報の反映
プロジェクトにpackrat情報が含まれている場合、プロジェクトを開いた際に`packrat/init.R`が自動的に実行され必要な処理が行われます。例えば最初にプロジェクトを開いた際には必要なパッケージをインストールする初期化処理が行われます。  
　  
...
Installing tidyverse (1.2.1) ... 
	OK (built source)
Packrat mode on. Using library in directory:
- "~/*/project_folder/packrat/lib"
```

　  

パッケージのインストールが完了すると上記のようなメッセージが表示されます。何らかのエラーが出たり初期化処理が止まったり場合には以下の関数を実行してください。  
　  
...
`packrat::restore`関数の処理が終了したら念の為にRStudioを再起動しプロジェクトを開き以下の関数を実行してください。  
...
### Windowsの場合
packratの初期化処理が完了してもRのセッションがCPUを数十パーセント専有し続ける場合があります。このような場合はRStudio（Rのセッション）を強制終了させてください。再起動後に以下の関数を実行して状態を確認してください。  
　  
...
## packrat情報の更新
プロジェクトのパッケージを更新したり追加、削除した場合は`packrat::snapshot`関数を用いてpackrat情報を更新します。更新された情報は`packrat/packrat.lock`ファイルに記録されますので、`packrat/packrat.lock`ファイルを更新してください。`packrat/packrat.lock`が更新されたらRStudioを再起動するか以下の関数を実行してください。  
　  
...
Snapshot written to 'project_path/packrat/packrat.lock'
```

この後、リモートリポジトリに変更結果（`packrat/packrat.lock`）をpushして変更を共有してください。  
　  

## ローカルリポジトリの更新
リモートリポジトリの更新を受けローカルリポジトリを更新したら`packrat::restore`関数で不要になったパッケージ情報を更新します。
```{r, eval=FALSE}
> packrat::restore()
...
Removing caTools ... OK
Removing clipr ... OK
Removing clisymbols ... OK
Removing desc ... OK
Removing enc ... OK
Removing gh ... OK
Removing git2r ... OK
Removing ini ... OK
Removing rematch2 ... OK
Removing styler ... OK
Removing usethis ... OK
```
...
## 起動に時間がかかる
packratは起動時にパッケージ情報をプロジェクト内のソースファイルを参照しながら確認しているようです。このため起動に時間がかかります。特にエラーがあるソースファイルがあると時間がかかりますので注意してください。  
　  

## メッセージが出続ける
`packrat::status`関数や`packrat::snapshot`関数を実行した場合に延々とメッセージを出し続ける場合があります。これはpackratが管理対象としているパッケージがコード内で使われているかをチェックするためにパッケージ単位でプロジェクトフォルダ内のファイルをチェックして（Rmdファイルの場合はkinitしている模様)おり、その際にファイルに記述されたコードにエラーがあるとエラー行をメッセージ表示し続けるためです。ちなみにエラーが出た場合、その行でチェックがabortされるのでエラー行の後でパッケージを呼び出しているような場合には正しいパッケージ情報を取得できない場合があるようです。  
　  
