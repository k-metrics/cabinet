...
## メッセージが出続ける
**`packrat::status`**関数や**`packrat::snapshot`**関数を実行した場合に延々とメッセージを出し続ける場合があります。これはpackratが管理対象としているパッケージがコード内で使われているかをチェックするためにパッケージ単位でプロジェクトフォルダ内のファイルをチェックして（Rmdファイルの場合はkinitしている模様)おり、その際にファイルに記述されたコードにエラーがあるとエラー行をメッセージ表示し続けるためです。ちなみにエラーが出た場合、その行でチェックabortされるようで
