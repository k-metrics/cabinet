...
# はじめに {#installPkg}
[R <i class="fa fa-external-link"></i>][R]はパッケージ（ライブラリ）を追加することで様々な分析に対応でき、機能も追加することができます。2017年4月1日の時点で10,000を超えるパッケージが[CRANに登録 <i class="fa fa-external-link"></i>](https://cran.ism.ac.jp/web/packages/){target="_blank" title="CRAN"}されています。  
...
![CRANミラーサイトの設定](./fig/pkg/Rcmdr_Install_01.jpg)
...
![HTTPS CRAN mirrors](./fig/pkg/Rcmdr_Install_02.jpg)
...
[R Commander(Rcmdr) <i class="fa fa-external-link"></i>][Rcmdr]のインストールは起動したRから行います。インストールの際はインターネット接続が必須です。プロキシサーバを設定する必要がある場合は[はじめに](#installPkg)を参照してください。


### パッケージのインストール
ミラーサイトの設定が終わりましたら、Rのメニューから［パッケージ］-［パッケージのインストール...］を選択します。

![パッケージのインストール](./fig/pkg/Rcmdr_Install_03.jpg)

別ウィンドウにパッケージ一覧が表示されますので`Rcmdr`を選択し、［OK］ボタンをクリックします。

![パッケージ選択](./fig/pkg/Rcmdr_Install_04.jpg)

#### Windowsの場合
UACによる制御がある場合には以下のようなダイアログが表示されますので、セキュリティポリシーにしたがってインストール先を決めて下さい。なお、UACについては[UACに関して](./env_install.html#UAC)をご覧ください。

![確認画面](./fig/pkg/Rcmdr_Install_05.jpg)

その他のダイアログが表示され、回答していくと[R Commander(Rcmdr) <i class="fa fa-external-link"></i>][Rcmdr]パッケージならびに関連パッケージがインストールされます。インストール中はRの画面にメッセージが表示されます。

![インストールログ](./fig/pkg/Rcmdr_Install_12.jpg)
...
![追加パッケージの確認](./fig/pkg/Rcmdr_Install_10.jpg)

パッケージのインストールが終わると`Rcmdr`が起動し別ウィンドウが表示されます。

![Rcmdrの起動](./fig/pkg/Rcmdr_Install_done.jpg)

#### Linuxの場合
Linux環境でRを利用している場合、[R <i class="fa fa-external-link"></i>][R]はターミナル（端末）上のコマンドラインとして実行されるためメニュー表示がありませんので、以下のコマンドを実行してください。
...
## R Commanderの動作確認
[R Commander(Rcmdr) <i class="fa fa-external-link"></i>][Rcmdr]が起動したら動作を確認しておきます。[R Commander(Rcmdr) <i class="fa fa-external-link"></i>][Rcmdr]のメニューから［データ］-［パッケージ内のデータ］-［アタッチされたパッケージからデータセットを読み込む...］を選択します。

![データセットの読み込み](./fig/pkg/Rcmdr_Check_01.jpg)

［パッケージからデータを読み込む］ダイアログが表示されたらパッケージ［datasets］をダブルクリックし、データセット［iris］をダブルクリックして選択します。

![データセットの選択](./fig/pkg/Rcmdr_Check_03.jpg)

データが読み込まれると[R Commander(Rcmdr) <i class="fa fa-external-link"></i>][Rcmdr]のメニューの下にある［データセット］欄に「iris」と表示されます。

![読み込んだデータセットの確認](./fig/pkg/Rcmdr_Check_04.jpg)

データセットを読み込んだらメニューから［グラフ］-［散布図］を選択します。

![散布図の描画](./fig/pkg/Rcmdr_Check_06.jpg)

下図のようなダイアログウィンドウが表示されますので、図のようにx変数とy変数を指定し［OK］ボタンをクリックします。

![散布図データの指定](./fig/pkg/Rcmdr_Check_08.jpg)

下図のようなグラフが描画されれば動作確認は完了です。

![散布図](./fig/pkg/Rcmdr_Check_09.jpg)
　  
