...
本資料は『[ソフトウェアメトリクス統計分析入門][BN2]』(以降、テキストと記載)の第2章の例題を[R][R]で解いた際の解答例を示したものです。また、[R Markdown][RM]を使うメリットを示すための一手段として作成していますので、テキストにおける計算方法とは異なる部分もあります。本資料で使用しているデータの入手に関してはテキストにてご確認下さい。  
...
最近では等分散性の検定（F検定）を行わず[ウェルチのt検定を行う方が妥当](http://laboratoryofbiology.blogspot.jp/2013/12/welchtanovastatwing.html)だと言われています。  
```{r}
t.test(values ~ group, alternative = "less", var.equal = FALSE, data = x.stacked)
```
等分散性に加えて正規性も仮定しない場合には[ブルンナー=ムンツェル検定（Brunner-Munzel Test）を使うのが妥当](http://d.hatena.ne.jp/hoxo_m/20150217/p1)だとされています。[R][R]でブルンナー=ムンツェル検定（Brunner-Munzel Test）を行うには**`lawstat`**パッケージをインストールしてください。  
```{r}
