...
### purrrパッケージによる結合
`lapply`関数と`do.call`関数の処理は`purrr::map_df`関数を用いることでよりシンプルに記述することができます。`purrr`パッケージの詳細については()[../tidy/purrr.html]をご覧ください。
```{r}
"./data/" %>% 
  list.files(path = ., pattern = "(issues_)", full.names = TRUE) %>% 
...
"./data/" %>% 
  list.files(path = ., pattern = "(issues_)", full.names = TRUE) %>% 
  # map_dfはmap_dfrのalias　map_dfr = lapply + do.call(bind_rows)
