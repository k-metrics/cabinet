...
**Import**は[Data Science Workflow, RStudio Inc.][RSH]において分析対象とたなるデータをインポートするデータ分析における最初のプロセスです。  
　  
...
[R][R]でインポートできるデータは概ね以下のように分類できます。  
　  
...
# ファイル
ファイルからのインポートは最も基本的なものです。[データ分析勉強会][0]などにおける演習で用いている[R Commander][Rcmdr]ではクリップボード経由で取り込むことが多かったですが、ファイルによる取り込みを憶えておくと利用範囲が広がります。  
　  
...
`foreign`    | SASやSPSSのような[R][R]以外の分析ソフトのファイルに対応
